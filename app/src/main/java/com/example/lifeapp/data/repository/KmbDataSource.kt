...
import com.example.trafficinquiry.data.model.TransitType
...
@Singleton
class KmbDataSource @Inject constructor() : TransitDataSource {

    override val company: OperatorCompany = OperatorCompany.KMB
    ...
    override suspend fun getRouteStops(
        ...
        val url = URL("https://data.etabus.gov.hk/v1/transport/kmb/route-stop/$routeName/$boundParam/$serviceType")
        val connection = url.openConnection() as HttpURLConnection
        try {
            val jsonStr = connection.inputStream.bufferedReader().use { it.readText() }
            val dataArray = JSONObject(jsonStr).getJSONArray("data")
    ...
    override suspend fun getEta(
        ...
        val url = URL("https://data.etabus.gov.hk/v1/transport/kmb/eta/$stopId/$routeName/$serviceType")
        val connection = url.openConnection() as HttpURLConnection
        try {
            val jsonStr = connection.inputStream.bufferedReader().use { it.readText() }
            val dataArray = JSONObject(jsonStr).getJSONArray("data")
            ...
            for (i in 0 until dataArray.length()) {
                ...
                val rName = obj.optString("route", routeName)
